...
class WC_Herepay_Payment_Gateway extends WC_Payment_Gateway {
    ...
    public function __construct() {
        ...
        $this->method_description = __('Herepay Payment Gateway integration for secure online payments.', 'woocommerce');
        ...
        $this->title = $this->get_option('title') ?: __('Herepay Payment Gateway', 'woocommerce');
        $this->description = $this->get_option('description') ?: __('Pay securely using Herepay Payment Gateway.', 'woocommerce');
    ...
    public function init_form_fields() {
        $this->form_fields = [
            ...
            'private_key' => [
                ...
                'type' => 'password',
                'description' => __('Enter your Herepay Private Key (used for checksum generation).', 'woocommerce'),
                'default' => ''
            ],
            'redirect_url' => [
                'title' => __('Redirect URL', 'woocommerce'),
                'type' => 'text',
                'description' => __('URL to redirect customers after payment completion. Use: ' . home_url('/herepay-redirect'), 'woocommerce'),
            ...
            'callback_url' => [
                ...
                'type' => 'text',
                'description' => __('Webhook URL for payment notifications. Use: ' . home_url('/wc-api/wc_herepay_payment_gateway'), 'woocommerce'),
    ...
    public function admin_options() {
        echo '<h3>' . __('Herepay Payment Gateway', 'woocommerce') . '</h3>';
        echo '<p>' . __('Configure your Herepay payment gateway settings below.', 'woocommerce') . '</p>';
    ...
    public function payment_fields() {
        ...
        if (empty($api_key) || empty($secret_key)) {
            ...
            if (class_exists('Herepay_Test_Config')) {
                ...
                echo '<p style="margin: 0; color: #856404;"><strong>' . __('Payment gateway is not fully configured.', 'woocommerce') . '</strong></p>';
                echo '<p style="margin: 5px 0 0 0; font-size: 12px; color: #856404;">' . __('Please contact the store administrator to configure Herepay payment settings.', 'woocommerce') . '</p>';
                echo '</div>';
                return;
            }
        }
        ...
        if (!$channels || !isset($channels['data']) || empty($channels['data'])) {
            ...
            echo '<p style="margin: 0; color: #721c24;"><strong>' . __('Unable to load payment channels.', 'woocommerce') . '</strong></p>';
            echo '<p style="margin: 5px 0 0 0; font-size: 12px; color: #721c24;">' . __('Please try again later or contact support if the problem persists.', 'woocommerce') . '</p>';
            echo '</div>';
            return;
        }
        ...
        echo '<label for="herepay_bank_prefix" style="display: block; margin-bottom: 5px; font-weight: bold;">' . __('Select Bank', 'woocommerce') . ' <span class="required" style="color: red;">*</span></label>';
        ...
        echo '<option value="">' . __('Select a bank...', 'woocommerce') . '</option>';
    ...
    public function process_payment($order_id) {
        ...
        // Update order status
        $order->update_status('pending', __('Awaiting Herepay payment confirmation.', 'woocommerce'));
    ...
    public function handle_callback() {
        ...
        if (isset($callback_data['checksum']) && !empty($this->private_key)) {
            ...
            if ($received_checksum !== $calculated_checksum) {
                ...
                $order->add_order_note(__('Herepay callback received with invalid checksum - possible security issue.', 'woocommerce'));
        ...
        if ($is_success) {
            ...
            if ($amount > 0 && abs($order->get_total() - $amount) > 0.01) {
                $order->add_order_note(sprintf(
                    __('Herepay payment amount mismatch. Expected: %s, Received: %s', 'woocommerce'),
                    $order->get_total(),
                    $amount
                ));
            ...
            $order->update_status('processing', __('Payment received via Herepay - processing order.', 'woocommerce'));
            $order->payment_complete();
            $order->add_order_note(__('Payment completed successfully via Herepay callback.', 'woocommerce'));
            ...
            case 'failed':
            case 'cancelled':
                $order->update_status('failed', __('Payment failed via Herepay.', 'woocommerce'));
            ...
            case 'pending':
            case 'processing':
                $order->update_status('pending', __('Payment is pending confirmation via Herepay.', 'woocommerce'));
                break;
                
            case '41': // Unauthorized
            case 'unauthorized':
                $order->update_status('failed', __('Payment authorization failed via Herepay.', 'woocommerce'));
                break;
                
            default:
                // Unknown status - log but don't change order status
                $order->add_order_note(sprintf(
                    __('Herepay callback received with unknown status: %s', 'woocommerce'),
                    $payment_status
                ));
    ...
    public function handle_herepay_redirect() {
        ...
        if (!isset($redirect_data['payment_code']) || empty($redirect_data['payment_code'])) {
            ...
            wc_add_notice(__('Invalid payment data received.', 'woocommerce'), 'error');
        ...
        if (isset($redirect_data['checksum']) && !empty($this->private_key)) {
            ...
            if ($received_checksum !== $calculated_checksum) {
                ...
                $order->add_order_note(__('Herepay redirect received with invalid checksum.', 'woocommerce'));
                
                // Redirect to order pay page with error
                wc_add_notice(__('Payment verification failed. Please try again.', 'woocommerce'), 'error');
        ...
        if ($is_success) {
            ...
            if (!$order->is_paid()) {
                ...
                $order->payment_complete();
                $order->add_order_note(__('Payment completed successfully via Herepay redirect.', 'woocommerce'));
        ...
        switch ($payment_status) {
            ...
            case 'failed':
            case 'cancelled':
                $order->update_status('failed', __('Payment failed via Herepay redirect.', 'woocommerce'));
                
                // Add error notice and redirect to checkout
                wc_add_notice(__('Payment was not successful. Please try again.', 'woocommerce'), 'error');
                wp_redirect($order->get_checkout_payment_url());
                exit;
            ...
            case 'pending':
            case 'processing':
                $order->update_status('pending', __('Payment is pending confirmation via Herepay redirect.', 'woocommerce'));
                
                // Redirect to order received page with pending message
                wc_add_notice(__('Your payment is being processed. You will receive confirmation once completed.', 'woocommerce'), 'notice');
                wp_redirect($order->get_checkout_order_received_url());
                exit;
                
            case '41': // Unauthorized
            case 'unauthorized':
                $order->update_status('failed', __('Payment authorization failed via Herepay redirect.', 'woocommerce'));
                
                // Add error notice and redirect to checkout
                wc_add_notice(__('Payment authorization failed. Please try again.', 'woocommerce'), 'error');
                wp_redirect($order->get_checkout_payment_url());
                exit;
            ...
            default:
                ...
                $order->add_order_note(sprintf(
                    __('Herepay redirect received with unknown status: %s', 'woocommerce'),
                    $payment_status
                ));
                
                wc_add_notice(__('Payment status is unclear. Please contact support if you have completed the payment.', 'woocommerce'), 'notice');
