...
class WC_Herepay_Payment_Gateway extends WC_Payment_Gateway {
    ...
    public function __construct() {
        ...
        $this->method_title = __('Herepay Payment Gateway', 'herepay-payment-gateway');
        $this->method_description = __('Herepay Payment Gateway integration for secure online payments.', 'herepay-payment-gateway');
        ...
        $this->title = $this->get_option('title') ?: __('Herepay Payment Gateway', 'herepay-payment-gateway');
        $this->description = $this->get_option('description') ?: __('Pay securely using Herepay Payment Gateway.', 'herepay-payment-gateway');
    ...
    public function init_form_fields() {
        $this->form_fields = [
            'enabled' => [
                'title' => __('Enable/Disable', 'herepay-payment-gateway'),
                'type' => 'checkbox',
                'label' => __('Enable Herepay Payment Gateway', 'herepay-payment-gateway'),
                'default' => 'yes'
            ],
            'title' => [
                'title' => __('Title', 'herepay-payment-gateway'),
                'type' => 'text',
                'description' => __('Payment method title shown at checkout.', 'herepay-payment-gateway'),
                'default' => __('Herepay Payment Gateway', 'herepay-payment-gateway')
            ],
            'description' => [
                'title' => __('Description', 'herepay-payment-gateway'),
                'type' => 'textarea',
                'description' => __('Description shown to customers at checkout.', 'herepay-payment-gateway'),
                'default' => __('Pay securely using Herepay Payment Gateway.', 'herepay-payment-gateway')
            ],
            'environment' => [
                'title' => __('Environment', 'herepay-payment-gateway'),
                'type' => 'select',
                'description' => __('Select the environment for your Herepay account.', 'herepay-payment-gateway'),
                'default' => 'sandbox',
                'options' => [
                    'sandbox' => __('Sandbox', 'herepay-payment-gateway'),
                    'production' => __('Production', 'herepay-payment-gateway')
                ]
            ],
            'api_key' => [
                'title' => __('API Key', 'herepay-payment-gateway'),
                'type' => 'text',
                'description' => __('Enter your Herepay API key.', 'herepay-payment-gateway'),
                'default' => ''
            ],
            'secret_key' => [
                'title' => __('Secret Key', 'herepay-payment-gateway'),
                'type' => 'password',
                'description' => __('Enter your Herepay Secret Key.', 'herepay-payment-gateway'),
                'default' => ''
            ],
            'private_key' => [
                'title' => __('Private Key', 'herepay-payment-gateway'),
                'type' => 'password',
                'description' => __('Enter your Herepay Private Key (used for checksum generation).', 'herepay-payment-gateway'),
                'default' => ''
            ],
            'redirect_url' => [
                'title' => __('Redirect URL', 'herepay-payment-gateway'),
                'type' => 'text',
                'description' => sprintf(__('URL to redirect customers after payment completion. Use: %s', 'herepay-payment-gateway'), home_url('/herepay-redirect')),
                'default' => home_url('/herepay-redirect'),
                'custom_attributes' => ['readonly' => 'readonly']
            ],
            'callback_url' => [
                'title' => __('Callback URL', 'herepay-payment-gateway'),
                'type' => 'text',
                'description' => sprintf(__('Webhook URL for payment notifications. Use: %s', 'herepay-payment-gateway'), home_url('/wc-api/wc_herepay_payment_gateway')),
    ...
    public function admin_options() {
        echo '<h3>' . __('Herepay Payment Gateway', 'herepay-payment-gateway') . '</h3>';
        echo '<p>' . __('Configure your Herepay payment gateway settings below.', 'herepay-payment-gateway') . '</p>';
    ...
    public function payment_fields() {
        ...
        if (empty($api_key) || empty($secret_key)) {
            ...
            if (class_exists('Herepay_Test_Config')) {
                ...
                echo '<p style="margin: 0; color: #856404;"><strong>' . __('Payment gateway is not fully configured.', 'herepay-payment-gateway') . '</strong></p>';
                echo '<p style="margin: 5px 0 0 0; font-size: 12px; color: #856404;">' . __('Please contact the store administrator to configure Herepay payment settings.', 'herepay-payment-gateway') . '</p>';
                echo '</div>';
                return;
            }
        }
        ...
        if (!$channels || !isset($channels['data']) || empty($channels['data'])) {
            ...
            echo '<p style="margin: 0; color: #721c24;"><strong>' . __('Unable to load payment channels.', 'herepay-payment-gateway') . '</strong></p>';
            echo '<p style="margin: 5px 0 0 0; font-size: 12px; color: #721c24;">' . __('Please try again later or contact support if the problem persists.', 'herepay-payment-gateway') . '</p>';
            echo '</div>';
            return;
        }
        ...
        echo '<label for="herepay_bank_prefix" style="display: block; margin-bottom: 5px; font-weight: bold;">' . __('Select Bank', 'herepay-payment-gateway') . ' <span class="required" style="color: red;">*</span></label>';
        ...
        echo '<option value="">' . __('Select a bank...', 'herepay-payment-gateway') . '</option>';
    ...
    public function validate_fields() {
        ...
        if (empty($bank_prefix)) {
            wc_add_notice(__('Please select a bank for payment.', 'herepay-payment-gateway'), 'error');
            return false;
        }
        
        if (empty($payment_method)) {
            wc_add_notice(__('Payment method not selected.', 'herepay-payment-gateway'), 'error');
            return false;
        }
    ...
    public function process_payment($order_id) {
        ...
        if (!$order) {
            wc_add_notice(__('Order not found.', 'herepay-payment-gateway'), 'error');
            return ['result' => 'fail'];
        }
        ...
        // Update order status
        $order->update_status('pending', __('Awaiting Herepay payment confirmation.', 'herepay-payment-gateway'));
    ...
    public function handle_callback() {
        ...
        if (isset($callback_data['checksum']) && !empty($this->private_key)) {
            ...
            if ($received_checksum !== $calculated_checksum) {
                ...
                $order->add_order_note(__('Herepay callback received with invalid checksum - possible security issue.', 'herepay-payment-gateway'));
        ...
        if ($is_success) {
            ...
            if ($amount > 0 && abs($order->get_total() - $amount) > 0.01) {
                $order->add_order_note(sprintf(
                    __('Herepay payment amount mismatch. Expected: %s, Received: %s', 'herepay-payment-gateway'),
                    $order->get_total(),
                    $amount
                ));
            ...
            $order->update_status('processing', __('Payment received via Herepay - processing order.', 'herepay-payment-gateway'));
            $order->payment_complete();
            $order->add_order_note(__('Payment completed successfully via Herepay callback.', 'herepay-payment-gateway'));
            ...
            case 'failed':
            case 'cancelled':
                $order->update_status('failed', __('Payment failed via Herepay.', 'herepay-payment-gateway'));
            ...
            case 'pending':
            case 'processing':
                $order->update_status('pending', __('Payment is pending confirmation via Herepay.', 'herepay-payment-gateway'));
                break;
                
            case '41': // Unauthorized
            case 'unauthorized':
                $order->update_status('failed', __('Payment authorization failed via Herepay.', 'herepay-payment-gateway'));
                break;
                
            default:
                // Unknown status - log but don't change order status
                $order->add_order_note(sprintf(
                    __('Herepay callback received with unknown status: %s', 'herepay-payment-gateway'),
                    $payment_status
                ));
    ...
    public function handle_herepay_redirect() {
        ...
        if (!isset($redirect_data['payment_code']) || empty($redirect_data['payment_code'])) {
            ...
            wc_add_notice(__('Invalid payment data received.', 'herepay-payment-gateway'), 'error');
        ...
        if (isset($redirect_data['checksum']) && !empty($this->private_key)) {
            ...
            if ($received_checksum !== $calculated_checksum) {
                ...
                $order->add_order_note(__('Herepay redirect received with invalid checksum.', 'herepay-payment-gateway'));
                
                // Redirect to order pay page with error
                wc_add_notice(__('Payment verification failed. Please try again.', 'herepay-payment-gateway'), 'error');
        ...
        if ($is_success) {
            ...
            if (!$order->is_paid()) {
                ...
                $order->payment_complete();
                $order->add_order_note(__('Payment completed successfully via Herepay redirect.', 'herepay-payment-gateway'));
        ...
        switch ($payment_status) {
            ...
            case 'failed':
            case 'cancelled':
                $order->update_status('failed', __('Payment failed via Herepay redirect.', 'herepay-payment-gateway'));
                
                // Add error notice and redirect to checkout
                wc_add_notice(__('Payment was not successful. Please try again.', 'herepay-payment-gateway'), 'error');
                wp_redirect($order->get_checkout_payment_url());
                exit;
            ...
            case 'pending':
            case 'processing':
                $order->update_status('pending', __('Payment is pending confirmation via Herepay redirect.', 'herepay-payment-gateway'));
                
                // Redirect to order received page with pending message
                wc_add_notice(__('Your payment is being processed. You will receive confirmation once completed.', 'herepay-payment-gateway'), 'notice');
                wp_redirect($order->get_checkout_order_received_url());
                exit;
                
            case '41': // Unauthorized
            case 'unauthorized':
                $order->update_status('failed', __('Payment authorization failed via Herepay redirect.', 'herepay-payment-gateway'));
                
                // Add error notice and redirect to checkout
                wc_add_notice(__('Payment authorization failed. Please try again.', 'herepay-payment-gateway'), 'error');
                wp_redirect($order->get_checkout_payment_url());
                exit;
            ...
            default:
                ...
                $order->add_order_note(sprintf(
                    __('Herepay redirect received with unknown status: %s', 'herepay-payment-gateway'),
                    $payment_status
                ));
                
                wc_add_notice(__('Payment status is unclear. Please contact support if you have completed the payment.', 'herepay-payment-gateway'), 'notice');
